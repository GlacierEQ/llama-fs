...
// Initialize protection before any other operations
...
async function moveFileToCategory(filePath, category) {
  ...
  try {
    // Use the protection system to ensure we don't touch system files
    if (protection.isSystemFile(filePath) || protection.isProgramFile(filePath)) {
      console.error(`Skipping protected file: ${fileName}`);
      return false;
    }
    
    // Continue with safe rename operation
...
// Periodically check for garbage collection
setInterval(() => {
  try {
    if (global.gc) {
      global.gc();
